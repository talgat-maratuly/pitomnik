    ...
    <>
      ...
      <div className="no-print fixed right-4 top-4 z-50">
        <button
          type="button"
          onClick={onClose}
          className="flex h-10 w-10 items-center justify-center rounded-full bg-white text-lg text-slate-700 shadow-md ring-1 ring-slate-200 hover:bg-slate-50"
          aria-label="Закрыть"
        >
          ✕
        </button>
      </div>
      <div className="qr-print-card fixed left-1/2 top-1/2 z-50 max-w-sm -translate-x-1/2 -translate-y-1/2">
      ...
      <div className="no-print fixed bottom-6 left-1/2 z-50 -translate-x-1/2">
        <button
          ...
          className="rounded-lg bg-emerald-700 px-6 py-2.5 text-sm font-semibold text-white shadow-lg hover:bg-emerald-800"
        >
          Печать
        </button>
      </div>
